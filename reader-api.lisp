...
(defun convert-clipboard-function (library-name &aux clipboard i-stream c-stream name-list doc-string)
	...
	(labels ((convert-case (prev-char i-stream &aux (cur-char (read-char i-stream nil)) (ap-char (char-downcase prev-char)) res-char (res-list '(nil nil))) 
		 ...
		 (read-parameter-list (i-stream &aux (i-char (read-char i-stream)))
		     (if (char/= #\) i-char)
  			 (append (cond ((char= #\space i-char) '(#\" #\"))
				       ((char= #\, i-char) nil)
				       ((char= #\return i-char) (read-char i-stream) '(#\" #\) #\( #\"))
				       (t (list i-char)))
	    ...
	    (with-output-to-string (o-stream)
		  ...
		  (setq doc-string (remove nil
					   (mapcar #'(lambda (in) (remove "" in :test #'equal))
						   (read-from-string
						    (concatenate 'string
								 (append '(#\( #\( #\")
									 (read-parameter-list (setq c-stream (make-concatenated-stream a-stream i-stream)))
									 '(#\" #\) #\))))))))
		  (close c-stream))
	      (setq c-stream nil
		    name-list (cons name-list (mapcar #'(lambda (in)
							    (setq c-stream (append c-stream (list (last in 2))))
	      ...
	      (close i-stream) 
	      (write-char #\" o-stream)
	      (mapc #'(lambda (in) (princ (format nil "~%~a" in) o-stream)) c-stream)
	      (write-char #\) o-stream)
	      (empty-clipboard)
	      (lisp-string-to-foreign (setq doc-string (get-output-stream-string o-stream)) clipboard (1+ (length doc-string)) :encoding :utf-8) 
	      (and (zerop (set-clipboard-data +cf-text+ (pointer-address clipboard))) (error-message)))))
    (close-clipboard))
